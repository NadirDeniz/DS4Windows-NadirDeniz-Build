...
namespace DS4WinWPF.DS4Forms.ViewModels
{
    public class LogViewModel
    {
        ...
        public LogViewModel(APP.ControlService service)
        {
            string version = APP.Global.exeversion;
            logItems.Add(new LogItem { Datetime = DateTime.Now, Message = $"APP version {version}" });
            logItems.Add(new LogItem { Datetime = DateTime.Now, Message = $"APP Assembly Architecture: {(Environment.Is64BitProcess ? "x64" : "x86")}" });
            logItems.Add(new LogItem { Datetime = DateTime.Now, Message = $"OS Version: {Environment.OSVersion}" });
            logItems.Add(new LogItem { Datetime = DateTime.Now, Message = $"OS Product Name: {APP.Util.GetOSProductName()}" });
            logItems.Add(new LogItem { Datetime = DateTime.Now, Message = $"OS Release ID: {APP.Util.GetOSReleaseId()}" });
            logItems.Add(new LogItem { Datetime = DateTime.Now, Message = $"System Architecture: {(Environment.Is64BitOperatingSystem ? "x64" : "x32")}" });

            //logItems.Add(new LogItem { Datetime = DateTime.Now, Message = "APP version 2.0" });
            ...
            APP.AppLogger.GuiLog += AddLogMessage;
        }
        ...
        private void AddLogMessage(object sender, APP.DebugEventArgs e)
        {
