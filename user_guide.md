## User Guide
After completing the initial setup of the required drivers and dependencies, you are greeted with the main APP tab, controllers.
...
The **Controller Readings** tab gives a live readout of the data from the joysticks being transmitted to APP
...
**Output Slots** shows which controllers that are connected are designated to the 8 slots that APP allows to be plugged in at one time. Here you can also select a controller and virtually plug and unplug it in.

### Settings
![Settings Screenshot](https://user-images.githubusercontent.com/75176311/232901882-6e271499-cdff-4f93-bfa5-921205c7fb69.PNG)

The **Settings** tab is where the settings for the APP application are. Options such as *Run at Startup*, *Start Minimized*, or *Show Notifications* live here.

**Disconenct from BT when Stopping** - Stops the bluetooth connection to the controllers when APP is quit

**Flash Lightbar at High Latency** - Flashes the DualShock 4's lightbar red when APP detects high input latency

**Quick Change** - Auto disables bluetooth when connecting a controller via USB

**Icon Choice** - Changes the Icon of the APP application

**App Theme** - Switch APP to Light or Dark mode
...
The **Log** tab is where you can look at all of the events that the APP application has encountered. There is also a button to export the log for debugging purposes.

## Device Detection Issue

If your DS4 is not detected by APP and the lightbar continues to
...
APP (text written after version 1.5.15) for various reasons,
...
Big Picture Mode or launching a game. This presents a problem for APP
...
use APP properly, you have to open Steam Big Picture Mode, navigate to
