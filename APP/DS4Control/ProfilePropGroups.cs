...
namespace APP
{
    ...
    public class StickDeadZoneInfo
    {
        ...
        public const bool DEFAULT_ADVANCED_RADIAL = true;
        public const double DEFAULT_INNER_ZONE_SOFTNESS = 0.18;
        public const int DEFAULT_OUTER_DEADZONE = 0;
        // Velocity-based inner-zone scaling makes the output magnitude depend on how fast the
        // stick is moved (same position -> different output), which reads as stair-stepping and
        // non-smooth diagonal/circular motion. Off by default; opt-in only.
        public const bool DEFAULT_DYNAMIC_DZ_SCALING = false;
        public const bool DEFAULT_ADAPTIVE_ANTIDEAD = true;
        // Center calibration learns a per-axis offset and subtracts it from every sample. A small
        // held lean near center is mislearned as "center" and rotates the angle of later diagonal
        // input (axis-snapping / "diagonal feels wrong"). Off by default; opt-in only.
        public const bool DEFAULT_DYNAMIC_CENTER_CAL = false;
        public const double DEFAULT_CENTER_CAL_STRENGTH = 0.38;
        ...
        /// <summary>Rotationally consistent radial deadzone with soft inner edge and split inner/outer zones.</summary>
        public bool advancedRadialProcessing = DEFAULT_ADVANCED_RADIAL;
        public double innerZoneSoftness = DEFAULT_INNER_ZONE_SOFTNESS;
        /// <summary>Independent outer ring (0–127) inside max zone — trims full-deflection before maxZone cap.</summary>
        public int outerDeadzone = DEFAULT_OUTER_DEADZONE;
        public bool dynamicDeadzoneScaling = DEFAULT_DYNAMIC_DZ_SCALING;
        public bool adaptiveAntiDeadzone = DEFAULT_ADAPTIVE_ANTIDEAD;
        public bool dynamicCenterCalibration = DEFAULT_DYNAMIC_CENTER_CAL;
        public double centerCalStrength = DEFAULT_CENTER_CAL_STRENGTH;
        ...
        public void Reset()
        {
            ...
            advancedRadialProcessing = DEFAULT_ADVANCED_RADIAL;
            innerZoneSoftness = DEFAULT_INNER_ZONE_SOFTNESS;
            outerDeadzone = DEFAULT_OUTER_DEADZONE;
            dynamicDeadzoneScaling = DEFAULT_DYNAMIC_DZ_SCALING;
            adaptiveAntiDeadzone = DEFAULT_ADAPTIVE_ANTIDEAD;
            dynamicCenterCalibration = DEFAULT_DYNAMIC_CENTER_CAL;
            centerCalStrength = DEFAULT_CENTER_CAL_STRENGTH;
            xAxisDeadInfo.Reset();
            yAxisDeadInfo.Reset();
        }
    }
    ...
    /// <summary>
    /// Adaptive micro-aim precision for low-range stick movement.
    /// </summary>
    public class MicroAimPrecisionInfo
    {
        public const bool DEFAULT_LS_ENABLED = false;
        // RS default OFF: Dynamic Stick Response (default ON) already consumes these curve
        // parameters internally; a separate enabled flag would imply a standalone path that never runs.
        public const bool DEFAULT_RS_ENABLED = false;
        public const double DEFAULT_PRECISION_ZONE = 0.15;
        public const double DEFAULT_TRANSITION_ZONE = 0.20;
        public const double DEFAULT_PRECISION_EXPONENT = 1.35;

        public bool enabled = DEFAULT_RS_ENABLED;
        public double precisionZone = DEFAULT_PRECISION_ZONE;
        public double transitionZone = DEFAULT_TRANSITION_ZONE;
        public double precisionExponent = DEFAULT_PRECISION_EXPONENT;

        public void Reset(bool isLeftStick)
        {
            enabled = isLeftStick ? DEFAULT_LS_ENABLED : DEFAULT_RS_ENABLED;
            precisionZone = DEFAULT_PRECISION_ZONE;
            transitionZone = DEFAULT_TRANSITION_ZONE;
            precisionExponent = DEFAULT_PRECISION_EXPONENT;
        }
    }

    /// <summary>
    /// Velocity-adaptive stick response: slow = precision, medium = balanced, fast = linear.
    /// </summary>
    public class DynamicStickResponseInfo
    {
        public const bool DEFAULT_LS_ENABLED = false;
        public const bool DEFAULT_RS_ENABLED = true;
        public const double DEFAULT_SLOW_SPEED = 1.5;
        public const double DEFAULT_FAST_SPEED = 12.0;
        public const double DEFAULT_SPEED_SMOOTHING = 0.55;
        public const double DEFAULT_HISTORY_WEIGHT = 0.35;
        public const double DEFAULT_PEAK_DECAY = 0.88;
        public const double DEFAULT_ACCEL_GAIN = 0.015;
        public const double DEFAULT_MAX_ACCEL_BOOST = 0.22;
        public const double DEFAULT_BALANCED_ZONE_SCALE = 0.75;
        public const double DEFAULT_BALANCED_EXPONENT_SCALE = 0.85;

        public bool enabled = DEFAULT_RS_ENABLED;
        public double slowSpeedThreshold = DEFAULT_SLOW_SPEED;
        public double fastSpeedThreshold = DEFAULT_FAST_SPEED;
        public double speedSmoothing = DEFAULT_SPEED_SMOOTHING;
        public double historyWeight = DEFAULT_HISTORY_WEIGHT;
        public double peakDecay = DEFAULT_PEAK_DECAY;
        public double accelerationGain = DEFAULT_ACCEL_GAIN;
        public double maxAccelBoost = DEFAULT_MAX_ACCEL_BOOST;
        public double balancedZoneScale = DEFAULT_BALANCED_ZONE_SCALE;
        public double balancedExponentScale = DEFAULT_BALANCED_EXPONENT_SCALE;

        public void Reset(bool isRightStick)
        {
            enabled = isRightStick ? DEFAULT_RS_ENABLED : DEFAULT_LS_ENABLED;
            slowSpeedThreshold = DEFAULT_SLOW_SPEED;
            fastSpeedThreshold = DEFAULT_FAST_SPEED;
            speedSmoothing = DEFAULT_SPEED_SMOOTHING;
            historyWeight = DEFAULT_HISTORY_WEIGHT;
            peakDecay = DEFAULT_PEAK_DECAY;
            accelerationGain = DEFAULT_ACCEL_GAIN;
            maxAccelBoost = DEFAULT_MAX_ACCEL_BOOST;
            balancedZoneScale = DEFAULT_BALANCED_ZONE_SCALE;
            balancedExponentScale = DEFAULT_BALANCED_EXPONENT_SCALE;
        }
    }

    /// <summary>
    /// TMR / Hall-effect noise gate without temporal smoothing.
    /// </summary>
    public class TmrHallStickInfo
    {
        public const bool DEFAULT_LS_ENABLED = false;
        // Default OFF: on standard 8-bit DS4/DualSense HID input the gate threshold sits below 1 LSB,
        // so it is a practical no-op. Kept for potential high-resolution input / future redesign.
        public const bool DEFAULT_RS_ENABLED = false;
        public const double DEFAULT_BASE_JITTER_THRESHOLD = 2.5;
        public const double DEFAULT_CENTER_STABILITY_RADIUS = 0.12;
        public const double DEFAULT_CENTER_THRESHOLD_BOOST = 1.5;
        public const double DEFAULT_CENTER_SNAP_RADIUS = 0.008;
        public const double DEFAULT_INTENT_SPEED_THRESHOLD = 0.85;
        public const double DEFAULT_INTENT_NORM_DELTA_THRESHOLD = 0.006;

        public bool enabled = DEFAULT_RS_ENABLED;
        public double baseJitterThreshold = DEFAULT_BASE_JITTER_THRESHOLD;
        public double centerStabilityRadius = DEFAULT_CENTER_STABILITY_RADIUS;
        public double centerThresholdBoost = DEFAULT_CENTER_THRESHOLD_BOOST;
        public double centerSnapRadius = DEFAULT_CENTER_SNAP_RADIUS;
        public double intentSpeedThreshold = DEFAULT_INTENT_SPEED_THRESHOLD;
        public double intentNormDeltaThreshold = DEFAULT_INTENT_NORM_DELTA_THRESHOLD;

        public void Reset(bool isRightStick)
        {
            enabled = isRightStick ? DEFAULT_RS_ENABLED : DEFAULT_LS_ENABLED;
            baseJitterThreshold = DEFAULT_BASE_JITTER_THRESHOLD;
            centerStabilityRadius = DEFAULT_CENTER_STABILITY_RADIUS;
            centerThresholdBoost = DEFAULT_CENTER_THRESHOLD_BOOST;
            centerSnapRadius = DEFAULT_CENTER_SNAP_RADIUS;
            intentSpeedThreshold = DEFAULT_INTENT_SPEED_THRESHOLD;
            intentNormDeltaThreshold = DEFAULT_INTENT_NORM_DELTA_THRESHOLD;
        }
    }

    /// <summary>
    /// Soft radial center-exit shaping for gradual movement away from center.
    /// </summary>
    public class SoftCenterExitInfo
    {
        public const bool DEFAULT_ENABLED = false;
        public const double DEFAULT_RANGE = 0.08;
        public const double DEFAULT_EXPONENT = 1.4;
        public const double DEFAULT_VELOCITY_BYPASS = 0.20;

        public bool enabled = DEFAULT_ENABLED;
        public double range = DEFAULT_RANGE;
        public double exponent = DEFAULT_EXPONENT;
        /// <summary>
        /// Legacy load-only field from the pre-Hermite velocity-bypass SCE. Retained for old profile
        /// XML compatibility; <see cref="SoftCenterExitProcessor"/> does not read this value.
        /// </summary>
        public double velocityBypass = DEFAULT_VELOCITY_BYPASS;
        ...
        public void Reset()
        {
            ...
            velocityBypass = DEFAULT_VELOCITY_BYPASS;
        }
    }

    /// <summary>
    /// In-game deadzone-aware radial remapper for the right stick.
    /// Maps post-physical-deadzone input so game movement starts just above the game's internal deadzone.
    /// </summary>
    public class InGameDeadzoneRemapperInfo
    {
        public const bool DEFAULT_ENABLED = false;
        public const double DEFAULT_GAME_DEADZONE_SIZE = 0.05;
        public const double DEFAULT_EXPONENT = 1.4;
        public const bool DEFAULT_SOFT_ENTRY = true;
        public const double DEFAULT_ENTRY_RANGE = 0.08;
        public const double MIN_GAME_DEADZONE_SIZE = 0.00;
        public const double MAX_GAME_DEADZONE_SIZE = 0.30;
        public const double MIN_EXPONENT = 0.5;
        public const double MAX_EXPONENT = 3.0;
        public const double MIN_ENTRY_RANGE = 0.01;
        public const double MAX_ENTRY_RANGE = 0.30;

        public bool enabled = DEFAULT_ENABLED;
        public double gameDeadzoneSize = DEFAULT_GAME_DEADZONE_SIZE;
        public double exponent = DEFAULT_EXPONENT;
        public bool softEntry = DEFAULT_SOFT_ENTRY;
        public double entryRange = DEFAULT_ENTRY_RANGE;

        public void Reset()
        {
            enabled = DEFAULT_ENABLED;
            gameDeadzoneSize = DEFAULT_GAME_DEADZONE_SIZE;
            exponent = DEFAULT_EXPONENT;
            softEntry = DEFAULT_SOFT_ENTRY;
            entryRange = DEFAULT_ENTRY_RANGE;
        }
    }

    /// <summary>
    /// Micro velocity quantization smoother for low-range right-stick output.
    /// Uses magnitude error diffusion (not frame averaging) to reduce visible camera stepping.
    /// </summary>
    public class MicroVelocityQuantizationSmootherInfo
    {
        public const bool DEFAULT_ENABLED = false;
        public const double DEFAULT_STRENGTH = 0.20;
        public const double DEFAULT_RANGE = 0.12;
        public const double MIN_STRENGTH = 0.01;
        public const double MAX_STRENGTH = 1.0;
        public const double MIN_RANGE = 0.01;
        public const double MAX_RANGE = 0.50;
    ...
    /// <summary>
    /// Soft radial anti-deadzone compensation (replaces classic anti-dead when enabled).
    /// </summary>
    public class SoftAntiDeadzoneInfo
    {
        public const bool DEFAULT_ENABLED = false;
        public const int DEFAULT_COMPENSATION_SIZE = 12;
        public const double DEFAULT_RAMP_ZONE = 0.25;
        public const double DEFAULT_EXPONENT = 2.2;

        public bool enabled = DEFAULT_ENABLED;
        public int compensationSize = DEFAULT_COMPENSATION_SIZE;
        public double rampZone = DEFAULT_RAMP_ZONE;
        public double exponent = DEFAULT_EXPONENT;

        public void Reset()
        {
            enabled = DEFAULT_ENABLED;
            compensationSize = DEFAULT_COMPENSATION_SIZE;
            rampZone = DEFAULT_RAMP_ZONE;
            exponent = DEFAULT_EXPONENT;
        }
    }
    ...
    public class AnalogMouseVectorEngineInfo
    {
        public const bool DEFAULT_ENABLED = false;
        public const double DEFAULT_SENSITIVITY = 1.0;
        public const double DEFAULT_EXPONENT = 1.5;
        public const double DEFAULT_DEADZONE = 0.02;
        public const bool DEFAULT_SUBPIXEL = true;
        public const double DEFAULT_ANGLE_SMOOTHING = 0.10;
        public const double DEFAULT_VELOCITY_BYPASS = 0.35;

        public bool enabled = DEFAULT_ENABLED;
        public double sensitivity = DEFAULT_SENSITIVITY;
        public double exponent = DEFAULT_EXPONENT;
        public double deadzone = DEFAULT_DEADZONE;
        public bool subpixel = DEFAULT_SUBPIXEL;
        public double angleSmoothing = DEFAULT_ANGLE_SMOOTHING;
        public double velocityBypass = DEFAULT_VELOCITY_BYPASS;
        ...
        public void Reset()
        {
            ...
            angleSmoothing = DEFAULT_ANGLE_SMOOTHING;
            velocityBypass = DEFAULT_VELOCITY_BYPASS;
        }
    }
