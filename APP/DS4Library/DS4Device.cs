    ...
    public class DS4Device
    {
        ...
        public virtual void StartUpdate()
        {
            ...
            if (ds4Input == null)
            {
                ...
                else if (!InputLatencyPipeline.InlineUsbOutput)
                {
                    ds4Output = new Thread(OutReportCopy);
                    ds4Output.Priority = InputLatencyPipeline.UsbOutputCopyThreadPriority;
                ...
                ds4Input.Priority = InputLatencyPipeline.InputThreadPriority;
        ...
        protected unsafe void performDs4Input()
        {
            unchecked
            {
                ...
                while (!exitInputThread)
                {
                    ...
                    if (!InputLatencyPipeline.ThrottleLatencyAveraging ||
                        (++latencyFrameCounter % InputLatencyPipeline.LATENCY_SAMPLE_INTERVAL) == 0)
                    {
                    ...
                    if (!InputLatencyPipeline.SkipPostReadSyncWait)
                    {
                        readWaitEv.Wait();
                    }
        ...
        private unsafe void PrepareOutputReportInner(ref bool change, ref bool haptime)
        {
            ...
            if (usingBT && (this.featureSet & VidPidFeatureSet.OnlyOutputData0x05) == 0)
            {
                ...
                outReportBuffer[1] = (byte)(0xC0 | InputLatencyPipeline.GetEffectiveBtPollRate(btPollRate)); // input report rate
        ...
        private void sendOutputReport(bool synchronous, bool force = false, bool quitOutputThreadOnError = true)
        {
            ...
            // Some gamepads don't support lightbar and rumble, so no need to write out anything (writeOut always fails, so APP would accidentally force quit the gamepad connection).
            ...
            if (synchronous)
            {
                if (haptime)
                {
                    ...
                    if (!usingBT && !InputLatencyPipeline.InlineUsbOutput)
                    {
            ...
            else
            {
                ...
                if (haptime)
                {
                    ...
                    if (!InputLatencyPipeline.InlineUsbOutput)
                    {
                        Monitor.Pulse(outReportBuffer);
                    }
                }
