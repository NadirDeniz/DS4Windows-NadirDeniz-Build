...
using APP.DS4Control;
using APP.InputDevices;
using APP.StickModifiers;
...
namespace APP
{
    ...
    public class Global
    {
        ...
        // Need to perform extra steps to check if APP is installed in a junction
        ...
        public static string exeversion = !string.IsNullOrWhiteSpace(fileVersion.ProductVersion)
            ? fileVersion.ProductVersion.Trim()
        ...
        public static string appDataPpath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\APP";
        public static string localAppDataPpath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "APP");
        ...
        public static OutContType[] outDevTypeTemp = new OutContType[TEST_PROFILE_ITEM_COUNT] { APP.OutContType.X360, APP.OutContType.X360,
            APP.OutContType.X360, APP.OutContType.X360,
            APP.OutContType.X360, APP.OutContType.X360,
            APP.OutContType.X360, APP.OutContType.X360,
            APP.OutContType.X360};
        // Used to hold the currently active controller output type in use for a slot
        public static OutContType[] activeOutDevType = new OutContType[TEST_PROFILE_ITEM_COUNT] { APP.OutContType.None, APP.OutContType.None,
            APP.OutContType.None, APP.OutContType.None,
            APP.OutContType.None, APP.OutContType.None,
            APP.OutContType.None, APP.OutContType.None,
            APP.OutContType.None};
        ...
        public const string ASSEMBLY_RESOURCE_PREFIX = "pack://application:,,,/APP;";
        public const string RESOURCES_PREFIX = "/APP;component/Resources";
        // Need to add additional probing path in code starting with .NET 6.
        public const string PROBING_PATH = "Lang";
        public const string LANGUAGE_ASSEMBLY_NAME = "APP.resources.dll";
        ...
        public static string getX360ControlString(X360Controls key, OutContType conType)
        {
            ...
            if (conType == APP.OutContType.X360)
            {
                xboxDefaultNames.TryGetValue(key, out result);
            }
            else if (conType == APP.OutContType.DS4)
            {
        ...
        {
            m_Config.ApplyFirstLaunchDefaults();
        }
        ...
        public static bool HasCompleteConfigAt(string basePath)
        {
            ...
            return File.Exists(Path.Combine(basePath, "Profiles.xml")) &&
                Directory.Exists(Path.Combine(basePath, "Profiles"));
        }
        ...
        public static void FindConfigLocation()
        {
            ...
            bool programConfigComplete = HasCompleteConfigAt(exedirpath);
            bool appDataConfigComplete = HasCompleteConfigAt(appDataPpath);

            bool programFolderAutoProfilesExists =
                File.Exists(Path.Combine(exedirpath, "Auto Profiles.xml"));
            bool appDataAutoProfilesExists =
                File.Exists(Path.Combine(appDataPpath, "Auto Profiles.xml"));

            bool isSameFolder = string.Equals(
                Path.GetFullPath(exedirpath),
                Path.GetFullPath(appDataPpath),
                StringComparison.OrdinalIgnoreCase);

            // Valid config in program folder only
            if (programConfigComplete && !appDataConfigComplete)
            {
                SaveWhere(exedirpath);
                return;
            }

            // Valid config in AppData (preferred when both exist)
            if (appDataConfigComplete)
            {
                SaveWhere(appDataPpath);
                return;
            }

            // Missing or incomplete config (e.g. Profiles.xml without Profiles folder)
            Global.firstRun = true;

            if (programFolderAutoProfilesExists &&
                appDataAutoProfilesExists &&
                !isSameFolder)
            {
        ...
        public static void CreateStdActions()
        {
            ...
            try
            {
                ...
                for (int i = 0, proflen = profiles.Length; i < proflen; i++)
                {
                    ...
                    {
                        xDoc.Load(s);
                        XmlNode el = xDoc.SelectSingleNode("APP/ProfileActions");
                        if (el != null)
                        {
                        ...
                        {
                            XmlNode Node = xDoc.SelectSingleNode("APP");
        ...
        public static bool LowLatencyInputMode
        {
            set { m_Config.lowLatencyInputMode = value; }
            get { return m_Config.lowLatencyInputMode; }
        }

        public static bool getLowLatencyInputMode()
        {
            return m_Config.lowLatencyInputMode;
        }
        ...
        /// <summary>
        /// Fake name used for user copy of APP.exe
        /// </summary>
        ...
        public static AnalogMouseVectorEngineInfo[] RSAnalogMouseVectorEngineInfos =>
            m_Config.rsAnalogMouseVectorEngineInfos;
        ...
        public static bool getCleanInputMode(int index)
        {
            return m_Config.cleanInputMode[index];
        }

        public static void setCleanInputMode(int index, bool value)
        {
            m_Config.cleanInputMode[index] = value;
        }

        public static bool[] CleanInputMode => m_Config.cleanInputMode;

        public static bool getCompetitiveFpsStickMode(int index)
        {
            return getCleanInputMode(index);
        }

        public static void setCompetitiveFpsStickMode(int index, bool value)
        {
            setCleanInputMode(index, value);
        }

        public static bool[] CompetitiveFpsStickMode => m_Config.cleanInputMode;

        public static bool getHighPrecisionStickOutput(int index)
        {
            return m_Config.highPrecisionStickOutput[index];
        }

        public static void setHighPrecisionStickOutput(int index, bool value)
        {
            m_Config.highPrecisionStickOutput[index] = value;
        }

        public static bool[] HighPrecisionStickOutput => m_Config.highPrecisionStickOutput;

        public static int getStickModifierOrder(int index)
        {
            return m_Config.stickModifierOrder[index];
        }

        public static void setStickModifierOrder(int index, int value)
        {
            m_Config.stickModifierOrder[index] =
                (int)StickModifierOrderPipeline.ClampOrder(value);
        }

        public static int[] StickModifierOrder => m_Config.stickModifierOrder;

        public static bool getTrue16BitStickPipeline(int index)
        {
            return m_Config.true16BitStickPipeline[index];
        }

        public static void setTrue16BitStickPipeline(int index, bool value)
        {
            m_Config.true16BitStickPipeline[index] = value;
        }

        public static bool getLSDirectionAwareResolution(int index)
        {
            return m_Config.lsDirectionAwareResolution[index];
        }

        public static void setLSDirectionAwareResolution(int index, bool value)
        {
            m_Config.lsDirectionAwareResolution[index] = value;
        }

        public static bool getRSDirectionAwareResolution(int index)
        {
            return m_Config.rsDirectionAwareResolution[index];
        }

        public static void setRSDirectionAwareResolution(int index, bool value)
        {
            m_Config.rsDirectionAwareResolution[index] = value;
        }

        public static bool getLSDynamicPhysicsResponse(int index)
        {
            return m_Config.lsDynamicPhysicsResponse[index];
        }

        public static void setLSDynamicPhysicsResponse(int index, bool value)
        {
            m_Config.lsDynamicPhysicsResponse[index] = value;
        }

        public static bool getRSDynamicPhysicsResponse(int index)
        {
            return m_Config.rsDynamicPhysicsResponse[index];
        }

        public static void setRSDynamicPhysicsResponse(int index, bool value)
        {
            m_Config.rsDynamicPhysicsResponse[index] = value;
        }

        public static bool getLSPredictiveJitterCleaner(int index)
        {
            return m_Config.lsPredictiveJitterCleaner[index];
        }

        public static void setLSPredictiveJitterCleaner(int index, bool value)
        {
            m_Config.lsPredictiveJitterCleaner[index] = value;
        }

        public static bool getRSPredictiveJitterCleaner(int index)
        {
            return m_Config.rsPredictiveJitterCleaner[index];
        }

        public static void setRSPredictiveJitterCleaner(int index, bool value)
        {
            m_Config.rsPredictiveJitterCleaner[index] = value;
        }
        ...
        public static MicroAimPrecisionInfo[] LSMicroAimPrecisionInfo => m_Config.lsMicroAimPrecisionInfo;
        public static MicroAimPrecisionInfo GetLSMicroAimPrecisionInfo(int device)
        {
            return m_Config.lsMicroAimPrecisionInfo[device];
        }

        public static MicroAimPrecisionInfo[] RSMicroAimPrecisionInfo => m_Config.rsMicroAimPrecisionInfo;
        public static MicroAimPrecisionInfo GetRSMicroAimPrecisionInfo(int device)
        {
            return m_Config.rsMicroAimPrecisionInfo[device];
        }

        public static DynamicStickResponseInfo[] LSDynamicStickResponseInfo => m_Config.lsDynamicStickResponseInfo;
        public static DynamicStickResponseInfo GetLSDynamicStickResponseInfo(int device)
        {
            return m_Config.lsDynamicStickResponseInfo[device];
        }

        public static DynamicStickResponseInfo[] RSDynamicStickResponseInfo => m_Config.rsDynamicStickResponseInfo;
        public static DynamicStickResponseInfo GetRSDynamicStickResponseInfo(int device)
        {
            return m_Config.rsDynamicStickResponseInfo[device];
        }

        public static TmrHallStickInfo[] LSTmrHallStickInfo => m_Config.lsTmrHallStickInfo;
        public static TmrHallStickInfo GetLSTmrHallStickInfo(int device)
        {
            return m_Config.lsTmrHallStickInfo[device];
        }

        public static TmrHallStickInfo[] RSTmrHallStickInfo => m_Config.rsTmrHallStickInfo;
        public static TmrHallStickInfo GetRSTmrHallStickInfo(int device)
        {
            return m_Config.rsTmrHallStickInfo[device];
        }

        public static SoftAntiDeadzoneInfo[] RSSoftAntiDeadzoneInfo => m_Config.rsSoftAntiDeadzoneInfo;
        public static SoftAntiDeadzoneInfo GetRSSoftAntiDeadzoneInfo(int device)
        {
            return m_Config.rsSoftAntiDeadzoneInfo[device];
        }

        public static InGameDeadzoneRemapperInfo[] RSInGameDeadzoneRemapperInfo => m_Config.rsInGameDeadzoneRemapperInfo;
        public static InGameDeadzoneRemapperInfo GetRSInGameDeadzoneRemapperInfo(int device)
        {
            return m_Config.rsInGameDeadzoneRemapperInfo[device];
        }

        public static MicroVelocityQuantizationSmootherInfo[] RSMicroVelocityQuantizationSmootherInfo =>
            m_Config.rsMicroVelocityQuantizationSmootherInfo;
        public static MicroVelocityQuantizationSmootherInfo GetRSMicroVelocityQuantizationSmootherInfo(int device)
        {
            return m_Config.rsMicroVelocityQuantizationSmootherInfo[device];
        }

        public static SoftCenterExitInfo[] LSSoftCenterExitInfo => m_Config.lsSoftCenterExitInfo;
        public static SoftCenterExitInfo GetLSSoftCenterExitInfo(int device)
        {
            return m_Config.lsSoftCenterExitInfo[device];
        }

        public static SoftCenterExitInfo[] RSSoftCenterExitInfo => m_Config.rsSoftCenterExitInfo;
        public static SoftCenterExitInfo GetRSSoftCenterExitInfo(int device)
        {
            return m_Config.rsSoftCenterExitInfo[device];
        }
    ...
    public class Changelog
    {
        ...
        public static bool CheckNewerVersionExists(out Version version, bool allowCached = true)
        {
            ...
            // Update checking is disabled in this build. Always report that no newer
            // version exists so the app never contacts the update server, never prompts
            // the user to update, and never downloads anything.
            return false;
...
#pragma warning restore CS0162 // Unreachable code detected
    ...
    public class BackingStore
    {
        ...
        public const bool DEFAULT_CLEAN_INPUT_MODE = true;
        public const bool DEFAULT_COMPETITIVE_FPS_STICK = DEFAULT_CLEAN_INPUT_MODE;
        public const bool DEFAULT_HIGH_PRECISION_STICK_OUTPUT = true;
        public const int DEFAULT_STICK_MODIFIER_ORDER =
            (int)StickModifierOrderMode.LegacyCompatible;
        public const bool DEFAULT_TRUE_16BIT_STICK_PIPELINE = false;
        public const bool DEFAULT_DIRECTION_AWARE_RESOLUTION = false;
        public const bool DEFAULT_DYNAMIC_PHYSICS_RESPONSE = false;
        public const bool DEFAULT_PREDICTIVE_JITTER_CLEANER = false;
        ...
        public AnalogMouseVectorEngineInfo[] rsAnalogMouseVectorEngineInfos =
            new AnalogMouseVectorEngineInfo[Global.TEST_PROFILE_ITEM_COUNT]
        {
            new AnalogMouseVectorEngineInfo(), new AnalogMouseVectorEngineInfo(),
            new AnalogMouseVectorEngineInfo(), new AnalogMouseVectorEngineInfo(),
            new AnalogMouseVectorEngineInfo(), new AnalogMouseVectorEngineInfo(),
            new AnalogMouseVectorEngineInfo(), new AnalogMouseVectorEngineInfo(),
            new AnalogMouseVectorEngineInfo(),
        };
        ...
        { DEFAULT_TOUCH_TOGGLE, DEFAULT_TOUCH_TOGGLE, DEFAULT_TOUCH_TOGGLE,
          DEFAULT_TOUCH_TOGGLE, DEFAULT_TOUCH_TOGGLE, DEFAULT_TOUCH_TOGGLE,
          DEFAULT_TOUCH_TOGGLE, DEFAULT_TOUCH_TOGGLE, DEFAULT_TOUCH_TOGGLE };
        public bool[] cleanInputMode = new bool[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_CLEAN_INPUT_MODE, DEFAULT_CLEAN_INPUT_MODE, DEFAULT_CLEAN_INPUT_MODE,
          DEFAULT_CLEAN_INPUT_MODE, DEFAULT_CLEAN_INPUT_MODE, DEFAULT_CLEAN_INPUT_MODE,
          DEFAULT_CLEAN_INPUT_MODE, DEFAULT_CLEAN_INPUT_MODE, DEFAULT_CLEAN_INPUT_MODE };
        public bool[] highPrecisionStickOutput = new bool[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_HIGH_PRECISION_STICK_OUTPUT, DEFAULT_HIGH_PRECISION_STICK_OUTPUT, DEFAULT_HIGH_PRECISION_STICK_OUTPUT,
          DEFAULT_HIGH_PRECISION_STICK_OUTPUT, DEFAULT_HIGH_PRECISION_STICK_OUTPUT, DEFAULT_HIGH_PRECISION_STICK_OUTPUT,
          DEFAULT_HIGH_PRECISION_STICK_OUTPUT, DEFAULT_HIGH_PRECISION_STICK_OUTPUT, DEFAULT_HIGH_PRECISION_STICK_OUTPUT };
        public int[] stickModifierOrder = new int[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_STICK_MODIFIER_ORDER, DEFAULT_STICK_MODIFIER_ORDER, DEFAULT_STICK_MODIFIER_ORDER,
          DEFAULT_STICK_MODIFIER_ORDER, DEFAULT_STICK_MODIFIER_ORDER, DEFAULT_STICK_MODIFIER_ORDER,
          DEFAULT_STICK_MODIFIER_ORDER, DEFAULT_STICK_MODIFIER_ORDER, DEFAULT_STICK_MODIFIER_ORDER };
        public bool[] true16BitStickPipeline = new bool[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_TRUE_16BIT_STICK_PIPELINE, DEFAULT_TRUE_16BIT_STICK_PIPELINE, DEFAULT_TRUE_16BIT_STICK_PIPELINE,
          DEFAULT_TRUE_16BIT_STICK_PIPELINE, DEFAULT_TRUE_16BIT_STICK_PIPELINE, DEFAULT_TRUE_16BIT_STICK_PIPELINE,
          DEFAULT_TRUE_16BIT_STICK_PIPELINE, DEFAULT_TRUE_16BIT_STICK_PIPELINE, DEFAULT_TRUE_16BIT_STICK_PIPELINE };
        public bool[] lsDirectionAwareResolution = new bool[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION,
          DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION,
          DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION };
        public bool[] rsDirectionAwareResolution = new bool[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION,
          DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION,
          DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION, DEFAULT_DIRECTION_AWARE_RESOLUTION };
        public bool[] lsDynamicPhysicsResponse = new bool[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE,
          DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE,
          DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE };
        public bool[] rsDynamicPhysicsResponse = new bool[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE,
          DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE,
          DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE, DEFAULT_DYNAMIC_PHYSICS_RESPONSE };
        public bool[] lsPredictiveJitterCleaner = new bool[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER,
          DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER,
          DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER };
        public bool[] rsPredictiveJitterCleaner = new bool[Global.TEST_PROFILE_ITEM_COUNT]
        { DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER,
          DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER,
          DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER, DEFAULT_PREDICTIVE_JITTER_CLEANER };
        ...
        public MicroAimPrecisionInfo[] lsMicroAimPrecisionInfo = CreateDefaultMicroAimPrecisionArray(false);
        public MicroAimPrecisionInfo[] rsMicroAimPrecisionInfo = CreateDefaultMicroAimPrecisionArray(true);
        public DynamicStickResponseInfo[] lsDynamicStickResponseInfo = CreateDefaultDynamicStickResponseArray(false);
        public DynamicStickResponseInfo[] rsDynamicStickResponseInfo = CreateDefaultDynamicStickResponseArray(true);
        public TmrHallStickInfo[] lsTmrHallStickInfo = CreateDefaultTmrHallStickArray(false);
        public TmrHallStickInfo[] rsTmrHallStickInfo = CreateDefaultTmrHallStickArray(true);
        public SoftAntiDeadzoneInfo[] rsSoftAntiDeadzoneInfo = CreateDefaultSoftAntiDeadzoneArray();
        public InGameDeadzoneRemapperInfo[] rsInGameDeadzoneRemapperInfo = CreateDefaultInGameDeadzoneRemapperArray();
        public MicroVelocityQuantizationSmootherInfo[] rsMicroVelocityQuantizationSmootherInfo =
            CreateDefaultMicroVelocityQuantizationSmootherArray();
        public SoftCenterExitInfo[] lsSoftCenterExitInfo = CreateDefaultSoftCenterExitArray();
        public SoftCenterExitInfo[] rsSoftCenterExitInfo = CreateDefaultSoftCenterExitArray();

        private static SoftCenterExitInfo[] CreateDefaultSoftCenterExitArray()
        {
            SoftCenterExitInfo[] array = new SoftCenterExitInfo[Global.TEST_PROFILE_ITEM_COUNT];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = new SoftCenterExitInfo();
                array[i].Reset();
            }
        ...
        private static SoftAntiDeadzoneInfo[] CreateDefaultSoftAntiDeadzoneArray()
        {
            SoftAntiDeadzoneInfo[] array = new SoftAntiDeadzoneInfo[Global.TEST_PROFILE_ITEM_COUNT];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = new SoftAntiDeadzoneInfo();
                array[i].Reset();
            }
        ...
        private static InGameDeadzoneRemapperInfo[] CreateDefaultInGameDeadzoneRemapperArray()
        {
            InGameDeadzoneRemapperInfo[] array = new InGameDeadzoneRemapperInfo[Global.TEST_PROFILE_ITEM_COUNT];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = new InGameDeadzoneRemapperInfo();
                array[i].Reset();
            }
        ...
        private static MicroVelocityQuantizationSmootherInfo[] CreateDefaultMicroVelocityQuantizationSmootherArray()
        {
            MicroVelocityQuantizationSmootherInfo[] array =
                new MicroVelocityQuantizationSmootherInfo[Global.TEST_PROFILE_ITEM_COUNT];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = new MicroVelocityQuantizationSmootherInfo();
                array[i].Reset();
            }
        ...
        private static TmrHallStickInfo[] CreateDefaultTmrHallStickArray(bool isRightStick)
        {
            TmrHallStickInfo[] array = new TmrHallStickInfo[Global.TEST_PROFILE_ITEM_COUNT];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = new TmrHallStickInfo();
                array[i].Reset(isRightStick);
            }

            return array;
        }

        private static DynamicStickResponseInfo[] CreateDefaultDynamicStickResponseArray(bool isRightStick)
        {
            DynamicStickResponseInfo[] array = new DynamicStickResponseInfo[Global.TEST_PROFILE_ITEM_COUNT];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = new DynamicStickResponseInfo();
                array[i].Reset(isRightStick);
            }
        ...
        private static MicroAimPrecisionInfo[] CreateDefaultMicroAimPrecisionArray(bool isRightStick)
        {
            MicroAimPrecisionInfo[] array = new MicroAimPrecisionInfo[Global.TEST_PROFILE_ITEM_COUNT];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = new MicroAimPrecisionInfo();
                array[i].Reset(isRightStick);
            }
        ...
        public Boolean startMinimized = true;
        ...
        public const int DEFAULT_NOTIFICATIONS = 0;
        ...
        public TrayIconChoice useIconChoice = TrayIconChoice.Default;
        public const bool DEFAULT_FLASH_WHEN_LATE = false;
        ...
        public bool lowLatencyInputMode = InputLatencyPipeline.DEFAULT_LOW_LATENCY_INPUT;
        ...
        public AppThemeChoice useCurrentTheme = AppThemeChoice.Dark;
        ...
        public bool SaveProfileOld(int device, string proName)
        {
            ...
            {
                XmlNode tmpNode;
                XmlNode xmlControls = m_Xdoc.SelectSingleNode("/APP/Control");
                XmlNode xmlShiftControls = m_Xdoc.SelectSingleNode("/APP/ShiftControl");
                m_Xdoc.RemoveAll();

                tmpNode = m_Xdoc.CreateXmlDeclaration("1.0", "utf-8", string.Empty);
                m_Xdoc.AppendChild(tmpNode);

                tmpNode = m_Xdoc.CreateComment(string.Format(" APP Configuration Data. {0} ", DateTime.Now));
                m_Xdoc.AppendChild(tmpNode);

                tmpNode = m_Xdoc.CreateComment(string.Format(" Made with APP version {0} ", Global.exeversion));
                m_Xdoc.AppendChild(tmpNode);

                tmpNode = m_Xdoc.CreateWhitespace("\r\n");
                m_Xdoc.AppendChild(tmpNode);

                XmlElement rootElement = m_Xdoc.CreateElement("APP", null);
                ...
                XmlNode xmlCleanInputMode = m_Xdoc.CreateNode(XmlNodeType.Element, "CleanInputMode", null); xmlCleanInputMode.InnerText = cleanInputMode[device].ToString(); rootElement.AppendChild(xmlCleanInputMode);
                XmlNode xmlHighPrecisionStick = m_Xdoc.CreateNode(XmlNodeType.Element, "HighPrecisionStickOutput", null); xmlHighPrecisionStick.InnerText = highPrecisionStickOutput[device].ToString(); rootElement.AppendChild(xmlHighPrecisionStick);
                XmlNode xmlStickModifierOrder = m_Xdoc.CreateNode(XmlNodeType.Element, "StickModifierOrder", null); xmlStickModifierOrder.InnerText = stickModifierOrder[device].ToString(); rootElement.AppendChild(xmlStickModifierOrder);
                XmlNode xmlTrue16Bit = m_Xdoc.CreateNode(XmlNodeType.Element, "True16BitStickPipeline", null); xmlTrue16Bit.InnerText = true16BitStickPipeline[device].ToString(); rootElement.AppendChild(xmlTrue16Bit);
                XmlNode xmlLsDirAware = m_Xdoc.CreateNode(XmlNodeType.Element, "LSDirectionAwareResolution", null); xmlLsDirAware.InnerText = lsDirectionAwareResolution[device].ToString(); rootElement.AppendChild(xmlLsDirAware);
                XmlNode xmlRsDirAware = m_Xdoc.CreateNode(XmlNodeType.Element, "RSDirectionAwareResolution", null); xmlRsDirAware.InnerText = rsDirectionAwareResolution[device].ToString(); rootElement.AppendChild(xmlRsDirAware);
                XmlNode xmlLsDynPhys = m_Xdoc.CreateNode(XmlNodeType.Element, "LSDynamicPhysicsResponse", null); xmlLsDynPhys.InnerText = lsDynamicPhysicsResponse[device].ToString(); rootElement.AppendChild(xmlLsDynPhys);
                XmlNode xmlRsDynPhys = m_Xdoc.CreateNode(XmlNodeType.Element, "RSDynamicPhysicsResponse", null); xmlRsDynPhys.InnerText = rsDynamicPhysicsResponse[device].ToString(); rootElement.AppendChild(xmlRsDynPhys);
                XmlNode xmlLsJit = m_Xdoc.CreateNode(XmlNodeType.Element, "LSPredictiveJitterCleaner", null); xmlLsJit.InnerText = lsPredictiveJitterCleaner[device].ToString(); rootElement.AppendChild(xmlLsJit);
                XmlNode xmlRsJit = m_Xdoc.CreateNode(XmlNodeType.Element, "RSPredictiveJitterCleaner", null); xmlRsJit.InnerText = rsPredictiveJitterCleaner[device].ToString(); rootElement.AppendChild(xmlRsJit);
                ...
                AdvancedDeadzoneProfileXml.SaveStick(m_Xdoc, rootElement, "LS", lsModInfo[device]);
                ...
                AdvancedDeadzoneProfileXml.SaveStick(m_Xdoc, rootElement, "RS", rsModInfo[device]);
                ...
                XmlNode xmlRSAnalogMouseVectorEngine = m_Xdoc.CreateNode(XmlNodeType.Element, "RSAnalogMouseVectorEngine", null); xmlRSAnalogMouseVectorEngine.InnerText = rsAnalogMouseVectorEngineInfos[device].enabled.ToString(); rootElement.AppendChild(xmlRSAnalogMouseVectorEngine);
                XmlNode xmlRSAnalogMouseVectorSensitivity = m_Xdoc.CreateNode(XmlNodeType.Element, "RSAnalogMouseVectorSensitivity", null); xmlRSAnalogMouseVectorSensitivity.InnerText = rsAnalogMouseVectorEngineInfos[device].sensitivity.ToString(); rootElement.AppendChild(xmlRSAnalogMouseVectorSensitivity);
                XmlNode xmlRSAnalogMouseVectorExponent = m_Xdoc.CreateNode(XmlNodeType.Element, "RSAnalogMouseVectorExponent", null); xmlRSAnalogMouseVectorExponent.InnerText = rsAnalogMouseVectorEngineInfos[device].exponent.ToString(); rootElement.AppendChild(xmlRSAnalogMouseVectorExponent);
                XmlNode xmlRSAnalogMouseVectorDeadzone = m_Xdoc.CreateNode(XmlNodeType.Element, "RSAnalogMouseVectorDeadzone", null); xmlRSAnalogMouseVectorDeadzone.InnerText = rsAnalogMouseVectorEngineInfos[device].deadzone.ToString(); rootElement.AppendChild(xmlRSAnalogMouseVectorDeadzone);
                XmlNode xmlRSAnalogMouseVectorSubpixel = m_Xdoc.CreateNode(XmlNodeType.Element, "RSAnalogMouseVectorSubpixel", null); xmlRSAnalogMouseVectorSubpixel.InnerText = rsAnalogMouseVectorEngineInfos[device].subpixel.ToString(); rootElement.AppendChild(xmlRSAnalogMouseVectorSubpixel);
                XmlNode xmlRSAnalogMouseVectorAngleSmoothing = m_Xdoc.CreateNode(XmlNodeType.Element, "RSAnalogMouseVectorAngleSmoothing", null); xmlRSAnalogMouseVectorAngleSmoothing.InnerText = rsAnalogMouseVectorEngineInfos[device].angleSmoothing.ToString(); rootElement.AppendChild(xmlRSAnalogMouseVectorAngleSmoothing);
                XmlNode xmlRSAnalogMouseVectorVelocityBypass = m_Xdoc.CreateNode(XmlNodeType.Element, "RSAnalogMouseVectorVelocityBypass", null); xmlRSAnalogMouseVectorVelocityBypass.InnerText = rsAnalogMouseVectorEngineInfos[device].velocityBypass.ToString(); rootElement.AppendChild(xmlRSAnalogMouseVectorVelocityBypass);
                ...
                */
                ...
                XmlNode xmlLsMicroAim = m_Xdoc.CreateNode(XmlNodeType.Element, "LSMicroAimPrecision", null); xmlLsMicroAim.InnerText = lsMicroAimPrecisionInfo[device].enabled.ToString(); rootElement.AppendChild(xmlLsMicroAim);
                XmlNode xmlRsMicroAim = m_Xdoc.CreateNode(XmlNodeType.Element, "RSMicroAimPrecision", null); xmlRsMicroAim.InnerText = rsMicroAimPrecisionInfo[device].enabled.ToString(); rootElement.AppendChild(xmlRsMicroAim);
                XmlNode xmlLsMicroAimZone = m_Xdoc.CreateNode(XmlNodeType.Element, "LSMicroAimPrecisionZone", null); xmlLsMicroAimZone.InnerText = lsMicroAimPrecisionInfo[device].precisionZone.ToString(); rootElement.AppendChild(xmlLsMicroAimZone);
                XmlNode xmlRsMicroAimZone = m_Xdoc.CreateNode(XmlNodeType.Element, "RSMicroAimPrecisionZone", null); xmlRsMicroAimZone.InnerText = rsMicroAimPrecisionInfo[device].precisionZone.ToString(); rootElement.AppendChild(xmlRsMicroAimZone);
                XmlNode xmlLsMicroAimTransition = m_Xdoc.CreateNode(XmlNodeType.Element, "LSMicroAimTransitionZone", null); xmlLsMicroAimTransition.InnerText = lsMicroAimPrecisionInfo[device].transitionZone.ToString(); rootElement.AppendChild(xmlLsMicroAimTransition);
                XmlNode xmlRsMicroAimTransition = m_Xdoc.CreateNode(XmlNodeType.Element, "RSMicroAimTransitionZone", null); xmlRsMicroAimTransition.InnerText = rsMicroAimPrecisionInfo[device].transitionZone.ToString(); rootElement.AppendChild(xmlRsMicroAimTransition);
                XmlNode xmlLsMicroAimExponent = m_Xdoc.CreateNode(XmlNodeType.Element, "LSMicroAimPrecisionExponent", null); xmlLsMicroAimExponent.InnerText = lsMicroAimPrecisionInfo[device].precisionExponent.ToString(); rootElement.AppendChild(xmlLsMicroAimExponent);
                XmlNode xmlRsMicroAimExponent = m_Xdoc.CreateNode(XmlNodeType.Element, "RSMicroAimPrecisionExponent", null); xmlRsMicroAimExponent.InnerText = rsMicroAimPrecisionInfo[device].precisionExponent.ToString(); rootElement.AppendChild(xmlRsMicroAimExponent);

                XmlNode xmlLsDynamicStick = m_Xdoc.CreateNode(XmlNodeType.Element, "LSDynamicStickResponse", null); xmlLsDynamicStick.InnerText = lsDynamicStickResponseInfo[device].enabled.ToString(); rootElement.AppendChild(xmlLsDynamicStick);
                XmlNode xmlRsDynamicStick = m_Xdoc.CreateNode(XmlNodeType.Element, "RSDynamicStickResponse", null); xmlRsDynamicStick.InnerText = rsDynamicStickResponseInfo[device].enabled.ToString(); rootElement.AppendChild(xmlRsDynamicStick);
                XmlNode xmlLsDynamicSlow = m_Xdoc.CreateNode(XmlNodeType.Element, "LSDynamicStickSlowSpeed", null); xmlLsDynamicSlow.InnerText = lsDynamicStickResponseInfo[device].slowSpeedThreshold.ToString(); rootElement.AppendChild(xmlLsDynamicSlow);
                XmlNode xmlRsDynamicSlow = m_Xdoc.CreateNode(XmlNodeType.Element, "RSDynamicStickSlowSpeed", null); xmlRsDynamicSlow.InnerText = rsDynamicStickResponseInfo[device].slowSpeedThreshold.ToString(); rootElement.AppendChild(xmlRsDynamicSlow);
                XmlNode xmlLsDynamicFast = m_Xdoc.CreateNode(XmlNodeType.Element, "LSDynamicStickFastSpeed", null); xmlLsDynamicFast.InnerText = lsDynamicStickResponseInfo[device].fastSpeedThreshold.ToString(); rootElement.AppendChild(xmlLsDynamicFast);
                XmlNode xmlRsDynamicFast = m_Xdoc.CreateNode(XmlNodeType.Element, "RSDynamicStickFastSpeed", null); xmlRsDynamicFast.InnerText = rsDynamicStickResponseInfo[device].fastSpeedThreshold.ToString(); rootElement.AppendChild(xmlRsDynamicFast);

                XmlNode xmlLsTmrHall = m_Xdoc.CreateNode(XmlNodeType.Element, "LSTmrHallStickMode", null); xmlLsTmrHall.InnerText = lsTmrHallStickInfo[device].enabled.ToString(); rootElement.AppendChild(xmlLsTmrHall);
                XmlNode xmlRsTmrHall = m_Xdoc.CreateNode(XmlNodeType.Element, "RSTmrHallStickMode", null); xmlRsTmrHall.InnerText = rsTmrHallStickInfo[device].enabled.ToString(); rootElement.AppendChild(xmlRsTmrHall);
                XmlNode xmlLsTmrJitter = m_Xdoc.CreateNode(XmlNodeType.Element, "LSTmrHallJitterThreshold", null); xmlLsTmrJitter.InnerText = lsTmrHallStickInfo[device].baseJitterThreshold.ToString(); rootElement.AppendChild(xmlLsTmrJitter);
                XmlNode xmlRsTmrJitter = m_Xdoc.CreateNode(XmlNodeType.Element, "RSTmrHallJitterThreshold", null); xmlRsTmrJitter.InnerText = rsTmrHallStickInfo[device].baseJitterThreshold.ToString(); rootElement.AppendChild(xmlRsTmrJitter);

                XmlNode xmlRsSoftAntiDead = m_Xdoc.CreateNode(XmlNodeType.Element, "RSSoftAntiDeadzone", null); xmlRsSoftAntiDead.InnerText = rsSoftAntiDeadzoneInfo[device].enabled.ToString(); rootElement.AppendChild(xmlRsSoftAntiDead);
                XmlNode xmlRsSoftAntiDeadSize = m_Xdoc.CreateNode(XmlNodeType.Element, "RSSoftAntiDeadzoneSize", null); xmlRsSoftAntiDeadSize.InnerText = rsSoftAntiDeadzoneInfo[device].compensationSize.ToString(); rootElement.AppendChild(xmlRsSoftAntiDeadSize);
                XmlNode xmlRsSoftAntiDeadRamp = m_Xdoc.CreateNode(XmlNodeType.Element, "RSSoftAntiDeadzoneRamp", null); xmlRsSoftAntiDeadRamp.InnerText = rsSoftAntiDeadzoneInfo[device].rampZone.ToString(); rootElement.AppendChild(xmlRsSoftAntiDeadRamp);
                XmlNode xmlRsSoftAntiDeadExponent = m_Xdoc.CreateNode(XmlNodeType.Element, "RSSoftAntiDeadzoneExponent", null); xmlRsSoftAntiDeadExponent.InnerText = rsSoftAntiDeadzoneInfo[device].exponent.ToString(); rootElement.AppendChild(xmlRsSoftAntiDeadExponent);

                XmlNode xmlRsInGameDeadzoneRemapper = m_Xdoc.CreateNode(XmlNodeType.Element, "RSInGameDeadzoneRemapper", null); xmlRsInGameDeadzoneRemapper.InnerText = rsInGameDeadzoneRemapperInfo[device].enabled.ToString(); rootElement.AppendChild(xmlRsInGameDeadzoneRemapper);
                XmlNode xmlRsInGameDeadzoneSize = m_Xdoc.CreateNode(XmlNodeType.Element, "RSInGameDeadzoneSize", null); xmlRsInGameDeadzoneSize.InnerText = rsInGameDeadzoneRemapperInfo[device].gameDeadzoneSize.ToString(); rootElement.AppendChild(xmlRsInGameDeadzoneSize);
                XmlNode xmlRsInGameDeadzoneExponent = m_Xdoc.CreateNode(XmlNodeType.Element, "RSInGameDeadzoneExponent", null); xmlRsInGameDeadzoneExponent.InnerText = rsInGameDeadzoneRemapperInfo[device].exponent.ToString(); rootElement.AppendChild(xmlRsInGameDeadzoneExponent);
                XmlNode xmlRsInGameDeadzoneSoftEntry = m_Xdoc.CreateNode(XmlNodeType.Element, "RSInGameDeadzoneSoftEntry", null); xmlRsInGameDeadzoneSoftEntry.InnerText = rsInGameDeadzoneRemapperInfo[device].softEntry.ToString(); rootElement.AppendChild(xmlRsInGameDeadzoneSoftEntry);
                XmlNode xmlRsInGameDeadzoneEntryRange = m_Xdoc.CreateNode(XmlNodeType.Element, "RSInGameDeadzoneEntryRange", null); xmlRsInGameDeadzoneEntryRange.InnerText = rsInGameDeadzoneRemapperInfo[device].entryRange.ToString(); rootElement.AppendChild(xmlRsInGameDeadzoneEntryRange);

                XmlNode xmlRsMicroVelocitySmoother = m_Xdoc.CreateNode(XmlNodeType.Element, "RSMicroVelocityQuantizationSmoother", null); xmlRsMicroVelocitySmoother.InnerText = rsMicroVelocityQuantizationSmootherInfo[device].enabled.ToString(); rootElement.AppendChild(xmlRsMicroVelocitySmoother);
                XmlNode xmlRsMicroVelocityStrength = m_Xdoc.CreateNode(XmlNodeType.Element, "RSMicroVelocitySmootherStrength", null); xmlRsMicroVelocityStrength.InnerText = rsMicroVelocityQuantizationSmootherInfo[device].strength.ToString(); rootElement.AppendChild(xmlRsMicroVelocityStrength);
                XmlNode xmlRsMicroVelocityRange = m_Xdoc.CreateNode(XmlNodeType.Element, "RSMicroVelocitySmootherRange", null); xmlRsMicroVelocityRange.InnerText = rsMicroVelocityQuantizationSmootherInfo[device].range.ToString(); rootElement.AppendChild(xmlRsMicroVelocityRange);

                XmlNode xmlLsSoftCenterExit = m_Xdoc.CreateNode(XmlNodeType.Element, "LSSoftCenterExit", null); xmlLsSoftCenterExit.InnerText = lsSoftCenterExitInfo[device].enabled.ToString(); rootElement.AppendChild(xmlLsSoftCenterExit);
                XmlNode xmlRsSoftCenterExit = m_Xdoc.CreateNode(XmlNodeType.Element, "RSSoftCenterExit", null); xmlRsSoftCenterExit.InnerText = rsSoftCenterExitInfo[device].enabled.ToString(); rootElement.AppendChild(xmlRsSoftCenterExit);
                XmlNode xmlLsSoftCenterExitRange = m_Xdoc.CreateNode(XmlNodeType.Element, "LSSoftCenterExitRange", null); xmlLsSoftCenterExitRange.InnerText = lsSoftCenterExitInfo[device].range.ToString(); rootElement.AppendChild(xmlLsSoftCenterExitRange);
                XmlNode xmlRsSoftCenterExitRange = m_Xdoc.CreateNode(XmlNodeType.Element, "RSSoftCenterExitRange", null); xmlRsSoftCenterExitRange.InnerText = rsSoftCenterExitInfo[device].range.ToString(); rootElement.AppendChild(xmlRsSoftCenterExitRange);
                XmlNode xmlLsSoftCenterExitExponent = m_Xdoc.CreateNode(XmlNodeType.Element, "LSSoftCenterExitExponent", null); xmlLsSoftCenterExitExponent.InnerText = lsSoftCenterExitInfo[device].exponent.ToString(); rootElement.AppendChild(xmlLsSoftCenterExitExponent);
                XmlNode xmlRsSoftCenterExitExponent = m_Xdoc.CreateNode(XmlNodeType.Element, "RSSoftCenterExitExponent", null); xmlRsSoftCenterExitExponent.InnerText = rsSoftCenterExitInfo[device].exponent.ToString(); rootElement.AppendChild(xmlRsSoftCenterExitExponent);
                // LSSoftCenterExitVelocityBypass / RSSoftCenterExitVelocityBypass: legacy load-only (Hermite SCE ignores).
            ...
            string propath = "", bool xinputChange = true, bool postLoad = true)
        {
            ...
            if (File.Exists(profilepath))
            {
                ...
                {
                    string detail = e.InnerException?.Message ?? e.Message;
                    AppLogger.LogToGui($"Failed to load {profilepath}. {detail}", false);
                    loaded = false;
                }
                catch (XmlException e)
                {
                    string detail = e.InnerException?.Message ?? e.Message;
                    AppLogger.LogToGui($"Failed to load {profilepath}. Invalid XML. {detail}", false);
                    loaded = false;
                }
            ...
            string propath = "", bool xinputChange = true, bool postLoad = true)
        {
            ...
            if (File.Exists(profilepath))
            {
                ...
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/CleanInputMode"); Boolean.TryParse(Item.InnerText, out cleanInputMode[device]); }
                catch
                {
                    try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/CompetitiveFpsStickMode"); Boolean.TryParse(Item.InnerText, out cleanInputMode[device]); }
                    catch { cleanInputMode[device] = DEFAULT_CLEAN_INPUT_MODE; missingSetting = true; }
                }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/HighPrecisionStickOutput"); Boolean.TryParse(Item.InnerText, out highPrecisionStickOutput[device]); }
                catch { highPrecisionStickOutput[device] = DEFAULT_HIGH_PRECISION_STICK_OUTPUT; missingSetting = true; }

                try
                {
                    Item = m_Xdoc.SelectSingleNode("/" + rootname + "/StickModifierOrder");
                    if (int.TryParse(Item?.InnerText ?? "", out int stickOrderRaw))
                    {
                        stickModifierOrder[device] = (int)StickModifierOrderPipeline.ClampOrder(stickOrderRaw);
                    }
                }
                catch { stickModifierOrder[device] = DEFAULT_STICK_MODIFIER_ORDER; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/True16BitStickPipeline"); Boolean.TryParse(Item.InnerText, out true16BitStickPipeline[device]); }
                catch { true16BitStickPipeline[device] = DEFAULT_TRUE_16BIT_STICK_PIPELINE; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSDirectionAwareResolution"); Boolean.TryParse(Item.InnerText, out lsDirectionAwareResolution[device]); }
                catch { lsDirectionAwareResolution[device] = DEFAULT_DIRECTION_AWARE_RESOLUTION; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSDirectionAwareResolution"); Boolean.TryParse(Item.InnerText, out rsDirectionAwareResolution[device]); }
                catch { rsDirectionAwareResolution[device] = DEFAULT_DIRECTION_AWARE_RESOLUTION; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSDynamicPhysicsResponse"); Boolean.TryParse(Item.InnerText, out lsDynamicPhysicsResponse[device]); }
                catch { lsDynamicPhysicsResponse[device] = DEFAULT_DYNAMIC_PHYSICS_RESPONSE; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSDynamicPhysicsResponse"); Boolean.TryParse(Item.InnerText, out rsDynamicPhysicsResponse[device]); }
                catch { rsDynamicPhysicsResponse[device] = DEFAULT_DYNAMIC_PHYSICS_RESPONSE; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSPredictiveJitterCleaner"); Boolean.TryParse(Item.InnerText, out lsPredictiveJitterCleaner[device]); }
                catch { lsPredictiveJitterCleaner[device] = DEFAULT_PREDICTIVE_JITTER_CLEANER; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSPredictiveJitterCleaner"); Boolean.TryParse(Item.InnerText, out rsPredictiveJitterCleaner[device]); }
                catch { rsPredictiveJitterCleaner[device] = DEFAULT_PREDICTIVE_JITTER_CLEANER; missingSetting = true; }
                ...
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSAnalogMouseVectorEngine"); Boolean.TryParse(Item.InnerText, out rsAnalogMouseVectorEngineInfos[device].enabled); }
                catch { rsAnalogMouseVectorEngineInfos[device].enabled = AnalogMouseVectorEngineInfo.DEFAULT_ENABLED; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSAnalogMouseVectorSensitivity"); double.TryParse(Item.InnerText, out rsAnalogMouseVectorEngineInfos[device].sensitivity); }
                catch { rsAnalogMouseVectorEngineInfos[device].sensitivity = AnalogMouseVectorEngineInfo.DEFAULT_SENSITIVITY; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSAnalogMouseVectorExponent"); double.TryParse(Item.InnerText, out rsAnalogMouseVectorEngineInfos[device].exponent); }
                catch { rsAnalogMouseVectorEngineInfos[device].exponent = AnalogMouseVectorEngineInfo.DEFAULT_EXPONENT; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSAnalogMouseVectorDeadzone"); double.TryParse(Item.InnerText, out rsAnalogMouseVectorEngineInfos[device].deadzone); }
                catch { rsAnalogMouseVectorEngineInfos[device].deadzone = AnalogMouseVectorEngineInfo.DEFAULT_DEADZONE; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSAnalogMouseVectorSubpixel"); Boolean.TryParse(Item.InnerText, out rsAnalogMouseVectorEngineInfos[device].subpixel); }
                catch { rsAnalogMouseVectorEngineInfos[device].subpixel = AnalogMouseVectorEngineInfo.DEFAULT_SUBPIXEL; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSAnalogMouseVectorAngleSmoothing"); double.TryParse(Item.InnerText, out rsAnalogMouseVectorEngineInfos[device].angleSmoothing); }
                catch { rsAnalogMouseVectorEngineInfos[device].angleSmoothing = AnalogMouseVectorEngineInfo.DEFAULT_ANGLE_SMOOTHING; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSAnalogMouseVectorVelocityBypass"); double.TryParse(Item.InnerText, out rsAnalogMouseVectorEngineInfos[device].velocityBypass); }
                catch { rsAnalogMouseVectorEngineInfos[device].velocityBypass = AnalogMouseVectorEngineInfo.DEFAULT_VELOCITY_BYPASS; missingSetting = true; }
                ...
                AdvancedDeadzoneProfileXml.LoadStick(m_Xdoc, rootname, "LS", lsModInfo[device]);
                AdvancedDeadzoneProfileXml.LoadStick(m_Xdoc, rootname, "RS", rsModInfo[device]);
                ...
                */
                ...
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSMicroAimPrecision"); Boolean.TryParse(Item.InnerText, out lsMicroAimPrecisionInfo[device].enabled); }
                catch { lsMicroAimPrecisionInfo[device].Reset(false); missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSMicroAimPrecision"); Boolean.TryParse(Item.InnerText, out rsMicroAimPrecisionInfo[device].enabled); }
                catch { rsMicroAimPrecisionInfo[device].Reset(true); missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSMicroAimPrecisionZone"); double.TryParse(Item.InnerText, out lsMicroAimPrecisionInfo[device].precisionZone); }
                catch { lsMicroAimPrecisionInfo[device].precisionZone = MicroAimPrecisionInfo.DEFAULT_PRECISION_ZONE; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSMicroAimPrecisionZone"); double.TryParse(Item.InnerText, out rsMicroAimPrecisionInfo[device].precisionZone); }
                catch { rsMicroAimPrecisionInfo[device].precisionZone = MicroAimPrecisionInfo.DEFAULT_PRECISION_ZONE; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSMicroAimTransitionZone"); double.TryParse(Item.InnerText, out lsMicroAimPrecisionInfo[device].transitionZone); }
                catch { lsMicroAimPrecisionInfo[device].transitionZone = MicroAimPrecisionInfo.DEFAULT_TRANSITION_ZONE; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSMicroAimTransitionZone"); double.TryParse(Item.InnerText, out rsMicroAimPrecisionInfo[device].transitionZone); }
                catch { rsMicroAimPrecisionInfo[device].transitionZone = MicroAimPrecisionInfo.DEFAULT_TRANSITION_ZONE; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSMicroAimPrecisionExponent"); double.TryParse(Item.InnerText, out lsMicroAimPrecisionInfo[device].precisionExponent); }
                catch { lsMicroAimPrecisionInfo[device].precisionExponent = MicroAimPrecisionInfo.DEFAULT_PRECISION_EXPONENT; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSMicroAimPrecisionExponent"); double.TryParse(Item.InnerText, out rsMicroAimPrecisionInfo[device].precisionExponent); }
                catch { rsMicroAimPrecisionInfo[device].precisionExponent = MicroAimPrecisionInfo.DEFAULT_PRECISION_EXPONENT; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSDynamicStickResponse"); Boolean.TryParse(Item.InnerText, out lsDynamicStickResponseInfo[device].enabled); }
                catch { lsDynamicStickResponseInfo[device].Reset(false); missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSDynamicStickResponse"); Boolean.TryParse(Item.InnerText, out rsDynamicStickResponseInfo[device].enabled); }
                catch { rsDynamicStickResponseInfo[device].Reset(true); missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSDynamicStickSlowSpeed"); double.TryParse(Item.InnerText, out lsDynamicStickResponseInfo[device].slowSpeedThreshold); }
                catch { lsDynamicStickResponseInfo[device].slowSpeedThreshold = DynamicStickResponseInfo.DEFAULT_SLOW_SPEED; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSDynamicStickSlowSpeed"); double.TryParse(Item.InnerText, out rsDynamicStickResponseInfo[device].slowSpeedThreshold); }
                catch { rsDynamicStickResponseInfo[device].slowSpeedThreshold = DynamicStickResponseInfo.DEFAULT_SLOW_SPEED; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSDynamicStickFastSpeed"); double.TryParse(Item.InnerText, out lsDynamicStickResponseInfo[device].fastSpeedThreshold); }
                catch { lsDynamicStickResponseInfo[device].fastSpeedThreshold = DynamicStickResponseInfo.DEFAULT_FAST_SPEED; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSDynamicStickFastSpeed"); double.TryParse(Item.InnerText, out rsDynamicStickResponseInfo[device].fastSpeedThreshold); }
                catch { rsDynamicStickResponseInfo[device].fastSpeedThreshold = DynamicStickResponseInfo.DEFAULT_FAST_SPEED; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSTmrHallStickMode"); Boolean.TryParse(Item.InnerText, out lsTmrHallStickInfo[device].enabled); }
                catch { lsTmrHallStickInfo[device].Reset(false); missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSTmrHallStickMode"); Boolean.TryParse(Item.InnerText, out rsTmrHallStickInfo[device].enabled); }
                catch { rsTmrHallStickInfo[device].Reset(true); missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSTmrHallJitterThreshold"); double.TryParse(Item.InnerText, out lsTmrHallStickInfo[device].baseJitterThreshold); }
                catch { lsTmrHallStickInfo[device].baseJitterThreshold = TmrHallStickInfo.DEFAULT_BASE_JITTER_THRESHOLD; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSTmrHallJitterThreshold"); double.TryParse(Item.InnerText, out rsTmrHallStickInfo[device].baseJitterThreshold); }
                catch { rsTmrHallStickInfo[device].baseJitterThreshold = TmrHallStickInfo.DEFAULT_BASE_JITTER_THRESHOLD; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSSoftAntiDeadzone"); Boolean.TryParse(Item.InnerText, out rsSoftAntiDeadzoneInfo[device].enabled); }
                catch { rsSoftAntiDeadzoneInfo[device].Reset(); missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSSoftAntiDeadzoneSize"); int.TryParse(Item.InnerText, out rsSoftAntiDeadzoneInfo[device].compensationSize); }
                catch { rsSoftAntiDeadzoneInfo[device].compensationSize = SoftAntiDeadzoneInfo.DEFAULT_COMPENSATION_SIZE; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSSoftAntiDeadzoneRamp"); double.TryParse(Item.InnerText, out rsSoftAntiDeadzoneInfo[device].rampZone); }
                catch { rsSoftAntiDeadzoneInfo[device].rampZone = SoftAntiDeadzoneInfo.DEFAULT_RAMP_ZONE; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSSoftAntiDeadzoneExponent"); double.TryParse(Item.InnerText, out rsSoftAntiDeadzoneInfo[device].exponent); }
                catch { rsSoftAntiDeadzoneInfo[device].exponent = SoftAntiDeadzoneInfo.DEFAULT_EXPONENT; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSInGameDeadzoneRemapper"); Boolean.TryParse(Item.InnerText, out rsInGameDeadzoneRemapperInfo[device].enabled); }
                catch { rsInGameDeadzoneRemapperInfo[device].Reset(); missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSInGameDeadzoneSize"); double.TryParse(Item.InnerText, out rsInGameDeadzoneRemapperInfo[device].gameDeadzoneSize); }
                catch { rsInGameDeadzoneRemapperInfo[device].gameDeadzoneSize = InGameDeadzoneRemapperInfo.DEFAULT_GAME_DEADZONE_SIZE; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSInGameDeadzoneExponent"); double.TryParse(Item.InnerText, out rsInGameDeadzoneRemapperInfo[device].exponent); }
                catch { rsInGameDeadzoneRemapperInfo[device].exponent = InGameDeadzoneRemapperInfo.DEFAULT_EXPONENT; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSInGameDeadzoneSoftEntry"); Boolean.TryParse(Item.InnerText, out rsInGameDeadzoneRemapperInfo[device].softEntry); }
                catch { rsInGameDeadzoneRemapperInfo[device].softEntry = InGameDeadzoneRemapperInfo.DEFAULT_SOFT_ENTRY; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSInGameDeadzoneEntryRange"); double.TryParse(Item.InnerText, out rsInGameDeadzoneRemapperInfo[device].entryRange); }
                catch { rsInGameDeadzoneRemapperInfo[device].entryRange = InGameDeadzoneRemapperInfo.DEFAULT_ENTRY_RANGE; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSMicroVelocityQuantizationSmoother"); Boolean.TryParse(Item.InnerText, out rsMicroVelocityQuantizationSmootherInfo[device].enabled); }
                catch { rsMicroVelocityQuantizationSmootherInfo[device].Reset(); missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSMicroVelocitySmootherStrength"); double.TryParse(Item.InnerText, out rsMicroVelocityQuantizationSmootherInfo[device].strength); }
                catch { rsMicroVelocityQuantizationSmootherInfo[device].strength = MicroVelocityQuantizationSmootherInfo.DEFAULT_STRENGTH; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSMicroVelocitySmootherRange"); double.TryParse(Item.InnerText, out rsMicroVelocityQuantizationSmootherInfo[device].range); }
                catch { rsMicroVelocityQuantizationSmootherInfo[device].range = MicroVelocityQuantizationSmootherInfo.DEFAULT_RANGE; missingSetting = true; }

                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSSoftCenterExit"); Boolean.TryParse(Item.InnerText, out lsSoftCenterExitInfo[device].enabled); }
                catch { lsSoftCenterExitInfo[device].Reset(); missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSSoftCenterExit"); Boolean.TryParse(Item.InnerText, out rsSoftCenterExitInfo[device].enabled); }
                catch { rsSoftCenterExitInfo[device].Reset(); missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSSoftCenterExitRange"); double.TryParse(Item.InnerText, out lsSoftCenterExitInfo[device].range); }
                catch { lsSoftCenterExitInfo[device].range = SoftCenterExitInfo.DEFAULT_RANGE; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSSoftCenterExitRange"); double.TryParse(Item.InnerText, out rsSoftCenterExitInfo[device].range); }
                catch { rsSoftCenterExitInfo[device].range = SoftCenterExitInfo.DEFAULT_RANGE; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSSoftCenterExitExponent"); double.TryParse(Item.InnerText, out lsSoftCenterExitInfo[device].exponent); }
                catch { lsSoftCenterExitInfo[device].exponent = SoftCenterExitInfo.DEFAULT_EXPONENT; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSSoftCenterExitExponent"); double.TryParse(Item.InnerText, out rsSoftCenterExitInfo[device].exponent); }
                catch { rsSoftCenterExitInfo[device].exponent = SoftCenterExitInfo.DEFAULT_EXPONENT; missingSetting = true; }
                // Legacy load-only: Hermite SCE does not use velocityBypass; tag kept for old profiles.
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSSoftCenterExitVelocityBypass"); double.TryParse(Item.InnerText, out lsSoftCenterExitInfo[device].velocityBypass); }
                catch { lsSoftCenterExitInfo[device].velocityBypass = SoftCenterExitInfo.DEFAULT_VELOCITY_BYPASS; missingSetting = true; }
                try { Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSSoftCenterExitVelocityBypass"); double.TryParse(Item.InnerText, out rsSoftCenterExitInfo[device].velocityBypass); }
                catch { rsSoftCenterExitInfo[device].velocityBypass = SoftCenterExitInfo.DEFAULT_VELOCITY_BYPASS; missingSetting = true; }
        ...
        public bool LoadOld()
        {
            ...
            try
            {
                if (File.Exists(m_Profile))
                {
                    ...
                    try { Item = m_Xdoc.SelectSingleNode("/Profile/LowLatencyInput"); Boolean.TryParse(Item.InnerText, out lowLatencyInputMode); }
                    catch { missingSetting = true; }
                    ...
                    if (xmlDeviceOptions != null)
                    {
                        ...
                        XmlNode xmlDS3Support = xmlDeviceOptions.SelectSingleNode("DS3SupportSettings");
                        if (xmlDS3Support != null)
                        {
                            try
                            {
                                XmlNode item = xmlDS3Support.SelectSingleNode("Enabled");
                                if (bool.TryParse(item?.InnerText ?? "", out bool temp))
                                {
                                    deviceOptions.DS3DeviceOpts.Enabled = temp;
                                }
                            }
                            catch { }
                        }
        ...
        public bool SaveOld()
        {
            ...
            Node = m_Xdoc.CreateComment(string.Format(" Made with APP version {0} ", Global.exeversion));
            m_Xdoc.AppendChild(Node);
            ...
            XmlNode xmlLowLatencyInput = m_Xdoc.CreateNode(XmlNodeType.Element, "LowLatencyInput", null); xmlLowLatencyInput.InnerText = lowLatencyInputMode.ToString(); rootElement.AppendChild(xmlLowLatencyInput);
            ...
            XmlElement xmlDS3Support = m_Xdoc.CreateElement("DS3SupportSettings", null);
            XmlElement xmlDS3Enabled = m_Xdoc.CreateElement("Enabled", null);
            xmlDS3Enabled.InnerText = deviceOptions.DS3DeviceOpts.Enabled.ToString();
            xmlDS3Support.AppendChild(xmlDS3Enabled);
            xmlDeviceOptions.AppendChild(xmlDS3Support);
        ...
        public void PostProcessLoad()
        {
            ...
            InputLatencyPipeline.ApplySystemTuning();
        }
        ...
        public void ApplyFirstLaunchDefaults()
        {
            ...
            flashWhenLate = DEFAULT_FLASH_WHEN_LATE;
            flashWhenLateAt = DEFAULT_FLASH_WHEN_LATE_AT;
            startMinimized = true;
            minToTaskbar = false;
            closeMini = true;
            quickCharge = false;
            disconnectBTAtStop = false;
            profileChangedNotification = false;
            useIconChoice = TrayIconChoice.Default;
            useCurrentTheme = AppThemeChoice.Dark;
            CheckWhen = DEFAULT_CHECK_WHEN;
            useOSCServ = false;
            oscServPort = DEFAULT_OSC_SERV_PORT;
            interpretingOscMonitoring = false;
            useOSCSend = false;
            oscSendPort = DEFAULT_OSC_SEND_PORT;
            oscSendAddress = DEFAULT_OSC_SEND_ADDRESS;
            useUDPServ = false;
            udpServPort = DEFAULT_UDP_SERV_PORT;
            udpServListenAddress = DEFAULT_UDP_SERV_LISTEN_ADDR;
            useUdpSmoothing = false;
            useCustomSteamFolder = false;
            customSteamFolder = null;
            fakeExeFileName = string.Empty;
            processPriority = 0;
            absDisplayEDID = string.Empty;
            useLang = string.Empty;
            downloadLang = false;
        }
        ...
        private void ResetProfile(int device)
        {
            ...
            rsAnalogMouseVectorEngineInfos[device].Reset();
            buttonAbsMouseInfos[device].Reset();
            gyroControlsInf[device].Reset();

            enableTouchToggle[device] = DEFAULT_TOUCH_TOGGLE;
            cleanInputMode[device] = DEFAULT_CLEAN_INPUT_MODE;
            highPrecisionStickOutput[device] = DEFAULT_HIGH_PRECISION_STICK_OUTPUT;
            stickModifierOrder[device] = DEFAULT_STICK_MODIFIER_ORDER;
            true16BitStickPipeline[device] = DEFAULT_TRUE_16BIT_STICK_PIPELINE;
            lsDirectionAwareResolution[device] = DEFAULT_DIRECTION_AWARE_RESOLUTION;
            rsDirectionAwareResolution[device] = DEFAULT_DIRECTION_AWARE_RESOLUTION;
            lsDynamicPhysicsResponse[device] = DEFAULT_DYNAMIC_PHYSICS_RESPONSE;
            rsDynamicPhysicsResponse[device] = DEFAULT_DYNAMIC_PHYSICS_RESPONSE;
            lsPredictiveJitterCleaner[device] = DEFAULT_PREDICTIVE_JITTER_CLEANER;
            rsPredictiveJitterCleaner[device] = DEFAULT_PREDICTIVE_JITTER_CLEANER;
            ...
            AdvancedDeadzoneProcessor.ResetDevice(device);
            PredictiveJitterCleanerProcessor.ResetDevice(device);
            SoftCenterExitProcessor.ResetDevice(device);
            MicroVelocityQuantizationSmootherProcessor.ResetDevice(device);
            DynamicStickResponseProcessor.ResetDevice(device);
            MicroAimPrecisionProcessor.ResetDevice(device);
            TmrHallStickProcessor.ResetDevice(device);
            ...
            lsMicroAimPrecisionInfo[device].Reset(false);
            rsMicroAimPrecisionInfo[device].Reset(true);
            lsDynamicStickResponseInfo[device].Reset(false);
            rsDynamicStickResponseInfo[device].Reset(true);
            lsTmrHallStickInfo[device].Reset(false);
            rsTmrHallStickInfo[device].Reset(true);
            rsSoftAntiDeadzoneInfo[device].Reset();
            rsInGameDeadzoneRemapperInfo[device].Reset();
            rsMicroVelocityQuantizationSmootherInfo[device].Reset();
            lsSoftCenterExitInfo[device].Reset();
            rsSoftCenterExitInfo[device].Reset();
