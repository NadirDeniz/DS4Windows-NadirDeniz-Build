    ...
    public partial class MainWindow : Window
    {
        ...
        public void LateChecks(ArgumentParser parser)
        {
            ...
            tempTask = Task.Delay(100).ContinueWith(_ =>
            {
                ...
                if (checkwhen > 0 && DateTime.Now >= Global.LastChecked + TimeSpan.FromHours(checkwhen))
                {
                    ...
                    {
                        Dispatcher.Invoke(() => MessageBox.Show(Strings.FailedToRetrieveLatestVersion, "APP Updater"));
        ...
        private void Check_Version(bool showstatus = false)
        {
            ...
            else
            {
                ...
                if (showstatus)
                {
                    Dispatcher.Invoke(() => MessageBox.Show(Properties.Resources.UpToDate, "APP Updater"));
                }
            }
        }
        ...
        private void ShowNotification(object sender, APP.DebugEventArgs e)
        {
            Dispatcher.BeginInvoke((Action)(() =>
            {
...
Suspend support not enabled.", true);
        ...
        private void ProfilesListBox_PreviewMouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            if (ItemsControl.ContainerFromElement(profilesListBox, source) is ListBoxItem)
            {
                return;
            }
        ...
        private void ControllerLV_PreviewMouseDown(object sender, MouseButtonEventArgs e)
        {
            if (e.ChangedButton != MouseButton.Left)
            {
                return;
            ...
            if (ItemsControl.ContainerFromElement(controllerLV, source) is ListViewItem)
            {
                return;
            }

            controllerLV.SelectedIndex = -1;
            conLvViewModel.CurrentIndex = -1;
        }

        private void RunAtStartCk_Click(object sender, RoutedEventArgs e)
        {
            settingsWrapVM.ShowRunStartPanel = settingsWrapVM.RunAtStartup ? Visibility.Visible :
            ...
            IntPtr lParam, ref bool handled)
        {
            ...
            switch (msg)
            {
                ...
                case WM_COPYDATA:
                {
                        ...
                        if (cds.cbData >= 4 && cds.cbData <= 256)
                        {
                            ...
                            if (strData.Length >= 1)
                            {
                                ...
                                else if (strData[0] == "query" && strData.Length >= 3)
                                {
                                    ...
                                    if (tdevice >= 0 && tdevice < ControlService.MAX_DS4_CONTROLLER_COUNT)
                                    {
                                        // Name of the property to query from a profile or APP app engine
                    ...
                    {
                        // Eat all exceptions in WM_COPYDATA because exceptions here are not fatal for APP background app
                    }
                    break;
                }
                default: break;
        ...
        {
            App.rootHub.ChangeOSCListenerStatus(settingsWrapVM.UseOSCServer);
        }

        private void UseOscSenderCk_Click(object sender, RoutedEventArgs e)
        {
            App.rootHub.ChangeOSCSenderStatus(settingsWrapVM.UseOSCSender);
        }

        private async void UseUdpServerCk_Click(object sender, RoutedEventArgs e)
        {
            bool status = settingsWrapVM.UseUDPServer;
            if (!status)
            {
        ...
        private void ImportProfBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            dialog.Filter = "APP Profile (*.xml)|*.xml";
        ...
        {
            ChangeHotkeysStatus(settingsWrapVM.SwipeTouchSwitchProfile);
        }
