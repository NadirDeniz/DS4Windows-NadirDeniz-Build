...
namespace APP
{
    public class Xbox360OutDevice : OutputDevice
    {
        ...
        private const float recipInputPosResolution = 1 / 127.5f;
        ...
        public override void ConvertandSendReport(DS4State state, int device)
        {
            ...
            switch (steeringWheelMappedAxis)
            {
                case SASteeringWheelEmulationAxisType.None:
                    if (state.UsePreciseStickOutput)
                    {
                        cont.LeftThumbX = state.PreciseLX;
                        cont.LeftThumbY = state.PreciseLY;
                        cont.RightThumbX = state.PreciseRX;
                        cont.RightThumbY = state.PreciseRY;
                    }
                    else
                    {
                ...
                case SASteeringWheelEmulationAxisType.VJoy2X:
                    APP.VJoyFeeder.vJoyFeeder.FeedAxisValue(state.SASteeringWheelEmulationUnit, ((((uint)steeringWheelMappedAxis) - ((uint)SASteeringWheelEmulationAxisType.VJoy1X)) / 3) + 1, APP.VJoyFeeder.HID_USAGES.HID_USAGE_X);
                    goto case SASteeringWheelEmulationAxisType.None;

                case SASteeringWheelEmulationAxisType.VJoy1Y:
                case SASteeringWheelEmulationAxisType.VJoy2Y:
                    APP.VJoyFeeder.vJoyFeeder.FeedAxisValue(state.SASteeringWheelEmulationUnit, ((((uint)steeringWheelMappedAxis) - ((uint)SASteeringWheelEmulationAxisType.VJoy1X)) / 3) + 1, APP.VJoyFeeder.HID_USAGES.HID_USAGE_Y);
                    goto case SASteeringWheelEmulationAxisType.None;

                case SASteeringWheelEmulationAxisType.VJoy1Z:
                case SASteeringWheelEmulationAxisType.VJoy2Z:
                    APP.VJoyFeeder.vJoyFeeder.FeedAxisValue(state.SASteeringWheelEmulationUnit, ((((uint)steeringWheelMappedAxis) - ((uint)SASteeringWheelEmulationAxisType.VJoy1X)) / 3) + 1, APP.VJoyFeeder.HID_USAGES.HID_USAGE_Z);
                    goto case SASteeringWheelEmulationAxisType.None;
        ...
        private short AxisScale(Int32 Value, Boolean Flip)
        {
            ...
            {
                Value -= 0x80;
                float temp = Value * recipInputPosResolution;
                if (Flip) temp = -temp;
                temp = (temp + 1.0f) * 0.5f;

                return (short)Math.Clamp(
                    (int)Math.Round(temp * outputResolution + (-32768), MidpointRounding.AwayFromZero),
                    -32768, 32767);
            }
        }
